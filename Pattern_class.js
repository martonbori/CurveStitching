class Pattern {
	...
	init() {
		var canvasWidth = myCanvas.getCanvas().clientWidth, canvasHeight = myCanvas.getCanvas().clientHeight;
		...
		for(var e=0;e<this.points.length;e++) {
			for(var n=0;n<this.points[e].length;n++) {
				for(var l=1;l<(parseInt(this.lineCntFromPoint)+1);l++) {
					...
					lVar.setValue(l);
					var endPointEdge=parseInt(this.endPointEdge(vars))%this.polygonN;
					var endPointNum=parseInt(this.endPointNum(vars))-1;
	...
	resetPolygon(polygonN=this.polygonN,pointsPerEdges=this.pointsPerEdges) {
